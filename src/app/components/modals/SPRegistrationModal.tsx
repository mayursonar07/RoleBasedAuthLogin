...
import { useSPRegistration } from "@/app/hooks/useSPRegistration";

interface SPRegistrationModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSwitchToLogin: () => void;
}

const SPRegistrationModal = ({ isOpen, onClose, onSwitchToLogin }: SPRegistrationModalProps) => {
    const {
        formData,
        step,
        loading,
    ...
    <Modal isOpen={isOpen} onClose={handleSkip}>
      <div className="bg-[#E8EDED] p-5 rounded-lg w-full max-w-lg md:max-w-md sm:max-w-sm mx-auto z-50 overflow-y-auto">
        <h2 className="text-lg md:text-base font-bold text-purple-700">
          {step === 1 ? "Register as Service Provider" : "Service provider registration successful..  Tell us more!"}
        </h2>
        ...
        {step === 1 && (
          <div className="mt-4 space-y-3">
            ...
            <div>
              <label className="block font-medium text-gray-700 text-xs">Business name</label>
              <input
                type="text"
                name="fullname"
...
export default SPRegistrationModal;
